...
def REDRESS_input_format(test_filename, output_filename, features,te, offset=0):	
	with open(test_filename, 'r') as f:
		test_data = [line.strip() for line in f if line.strip()]
	if not te:
		te = len(test_data)

	test_data = test_data[offset:offset+te]
	print('-----------------------------------------------------------------------')
...
def Generate_IncEncSetup_H(arr, method_name):
	setup_file = open(f"IncludeEncodedSetup_{method_name}.h", 'w')
	lines = [
		f"#ifndef INCLUDEENCODEDSETUP_{method_name.upper()}_H",
		f"#define INCLUDEENCODEDSETUP_{method_name.upper()}_H",
		f"struct IncludeEncodedSetup_{method_name} {{",
		f"    static constexpr int THRESHOLD = {arr[0]};",
		f"    static constexpr int CLAUSES = {arr[1]};",
		f"    static constexpr int CLASSES = {arr[2]};",
		f"    static constexpr int FEATURES = {arr[3]};",
		f"    static constexpr int NUMBER_OF_STATES = {arr[4]};",
		f"    static constexpr int NUMBER_OF_TEST_EXAMPLES = {arr[5]};",
		f"    static constexpr int TEST_LENGTH_PER32 = {arr[11]};",
		f"    static constexpr const char* TEST_FEATURE_FILE = \"{arr[6]}\";",
		f"    static constexpr const char* TEST_CLASSIFICATION_FILE = \"{arr[7]}\";",
		f"    static constexpr int NUM_OF_INCLUDES = {arr[8]};",
		f"    static constexpr const char* INCENC_TA_FILE = \"{arr[9]}\";",
		f"    static constexpr const char* INC_PER_CLASS_FILE = \"{arr[10]}\";",
		"    static constexpr int CLAUSE_POLARITY_BIT = 15;",  # fixed value
		"    static constexpr int BIT_DEMARCATING_CLAUSE_CHANGE = 14;",  # fixed value
		"    static constexpr int INT_VALUE_OF_ENCODING_BITS = 16383;",  # fixed value
		"};",
		"#endif"
	]
...
def main():
	...
	parser.add_argument("method", help="Method of feature vectorization", type=str)
	...
	parser.add_argument("-offset", help="Offset (starting index) for test examples. Default: 0", type=int, default=0)
	...
	if(args.method):
		...
		print('ERROR: Method needs to be specified, please check help.')
		sys.exit()
	...
	REDRESS_test_data, test_feature_filename, classification_filename, te, classification_output = REDRESS_input_format(args.test_filename, args.dataset_name + '_REDRESS_', args.features, args.te, args.offset)
	...
	Generate_IncEncSetup_H(IncEncSetup_array, method_name)
